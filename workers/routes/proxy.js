...
export async function handleBonusMinerProxy({ url, request, corsHeaders }) {
  if (!url.pathname.startsWith("/bm/")) return null;

  const target = new URL("https://api.bonus-miner.gomining.com" + url.pathname.slice(3) + url.search);
